...
public class HomeController {
	...
	public void bookDetails(@RequestParam(name = "id") int id, Model model) {
		...
		Book book = Book.builder().id(id).title("종의 기원").author(Author.builder().firstName("찰스").lastName("다윈").build())
				.build();
		model.addAttribute("book", book);
	}
	...
	public String bookDetails2(@PathVariable(name = "id") int id, Model model) {
		...
		Book book = Book.builder().id(id).title("종의 기원").author(Author.builder().firstName("유정").lastName("정").build())
				.build();
		model.addAttribute("book", book);
		return "book/details";
